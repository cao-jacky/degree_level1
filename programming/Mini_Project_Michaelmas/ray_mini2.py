...
def refraction_2d (incident_rays, planar_surface):
    '''A function which can calculate the location of a refracted ray and also the angle the refracted ray makes with the vertical.'''
    ...
    critical_angle = numpy.arcsin((ps_r2 / ps_r1))
    print critical_angle #test

    if any(a[:,2]) >= critical_angle: #simple code to terminate the function if the angles given are greater than the critical angle
        print "Hawk! An exception has been rasied:"
        print "An incident angle (or multiple angles) is greater than critical angle, function has been terminated."
        return
    else:
        print "Incident angles are valid, function will proceed." #'useful' text-based feedback to show if the function is actually working or not
    ...
    ar = numpy.arcsin((ps_r1 / ps_r2) * numpy.sin(ai))
    print "ar:"
    print ar
