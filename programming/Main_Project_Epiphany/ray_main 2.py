...
def plot_trace_2d (incident_rays, refracted_ray_paths, surface_list):
    ...
    for i in range(len(surface_list)):
        if surface_list[i][0] == 'SPH': #Plotting spherical surface
            ...
            x = numpy.linspace(0,surface_list[i][1][0],100)
            y = numpy.sqrt((surface_list[i][1][6] ** 2) - ((x - refraction_2d_sph.ccc) ** 2))
...
def evaluate_trace_2d (refracted_ray_paths, r):
    ...
    rrp_average = numpy.average(rrpf)

    #Adding r above the average value
    rrp_average_plus = rrp_average + r
    #Subtracting r below the average value
    rrp_average_minus = rrp_average - r

    print rrpf, rrp_average, rrp_average_plus, rrp_average_minus
    ...
    for i in rrpf:
        if (i >= rrp_average_minus) and (i <= rrp_average_plus):
...
if __name__ == '__main__':
    ...
    #ray_main.refraction_2d(numpy.array([[0.0,5.0, numpy.pi/2.0],[0.0,5.0, 1.1*numpy.pi/2.0]]), numpy.array([5.0,2.0,6.0,8.0,1.0,1.33]))
    #ray_main.refraction_2d_sph(numpy.array([[0.0,3.0, numpy.pi/2.0],[0.0,3.0, 1.1*numpy.pi/2.0]]), numpy.array([6.0,4.0,6.0,-4.0,1.0,1.33,4.0]))
    ...
    #ray_main.trace_2d(numpy.array([[0.0,5.0, numpy.pi/2.0],[0.0,5.0, 1.1*numpy.pi/2.0]]), (["PLA", numpy.array([5.0,2.0,6.0,8.0,1.0,1.33])], ["SPH", numpy.array([6.0,4.0,6.0,-4.0,1.0,1.33,4.0])],['DET', 15.0]))
    #ray_main.trace_2d(numpy.array([[0.0,3.0, numpy.pi/2.0],[0.0,3.0, 1.1*numpy.pi/2.0]]), (["SPH", numpy.array([6.0,4.0,6.0,-4.0,1.0,1.33,4.0])], ["PLA", numpy.array([7.0,2.0,6.0,8.0,1.0,1.33])],["DET", 6.0]))

    ray_main.plot_trace_2d(numpy.array([[0.0,3.0, numpy.pi/2.0],[0.0,3.0, 1.1*numpy.pi/2.0]]), trace_2d, (["SPH",numpy.array([3.0,4.0,3.0,-4.0,1.4,1.48,5])], ["PLA",numpy.array([5.0,2.0,6.0,8.0,1.0,1.33])],["DET",15.0]))
